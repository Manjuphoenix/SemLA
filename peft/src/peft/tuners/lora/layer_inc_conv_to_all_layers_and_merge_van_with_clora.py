...
######################################################### EOF CONV #########################################################
    ...
    #         adapters_key = list(self.lora_A.keys())

    #         import ipdb; ipdb.set_trace()
    #         self.active_adapters.append(adapters_key[:-2])
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if active_adapter not in self.lora_variant:  # vanilla LoRA
                    ...
                    L_orig, B_orig, C_orig = la.shape
                    #   one final permute ...
                    la = la.permute(1, 2, 0)    #     #torch.Size([2, 8, 577])  and for text this will be (19, 48, 77)
                    ...
                    if la.shape[0]==5 and la.shape[2]==577 and "conv" in active_adapter:
                        ...
                        scaled_lora = lb * scaling
                        result = result + scaled_lora
                    elif la.shape[0]==19  and la.shape[2]==77 and "conv" in active_adapter:
                        # la.shape will be torch.Size([19, 48, 77])
                        # la = la.permute(0,2,1)
                        target_size = int(math.sqrt(L_orig))
                        target_spatial_size = target_size * target_size     #  64 (8 *8)
                        pooled = F.adaptive_avg_pool1d(
                            la,  # [2, 8, 577]
                            target_spatial_size  # Pool to 576
                        ).permute(0, 2, 1)  # [2, 576, 8]
                        ...
                        scaled_lora = lb * scaling
                        result = result + scaled_lora
                    elif la.shape[0]==576 and la.shape[2]==95 and "conv" in active_adapter:
                        # la.shape will be torch.Size([576, 48, 95])
                        L_orig, B_orig, C_orig = la.shape
                        target_size = int(math.sqrt(L_orig))
                        target_spatial_size = target_size * target_size
                        pooled = F.adaptive_avg_pool1d(
                            la.permute(2, 1, 0),  # [95, 48, 576]
                            target_spatial_size  # Pool to 576
                        ).permute(0, 2, 1)  # [95, 576, 48]
                        ...
                        lb = lora_B(conv_result)
                        scaled_lora = lb * scaling
                        scaled_lora = scaled_lora.permute(1, 0, 2)
                        result = result + scaled_lora


                    elif la.shape[0]==144 and la.shape[2]==380 and "conv" in active_adapter:
                        # la.shape will be torch.Size([144, 48, 380])
                        L_orig, B_orig, C_orig = la.shape
                        target_size = int(math.sqrt(L_orig))
                        target_spatial_size = target_size * target_size
                        pooled = F.adaptive_avg_pool1d(

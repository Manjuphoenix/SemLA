...
class Linear(nn.Module, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ############### conv layer for LoRA ###########
        self.conv1 = nn.Conv2d(r, r, kernel_size=3, stride=2, padding=1, bias=False)
    ...
    # Fixed version with shape checks and proper reshaping
    def validate_and_reshape_lora(self, la):
        """
        Handles reshaping of LoRA features with proper padding and reshaping for non-perfect square sequences
        
        Args:
            la (torch.Tensor): Input tensor of shape [L, B, D] where:
                L: sequence length (e.g., 577)
                B: batch size (e.g., 2)
                D: feature dimension (e.g., 8)
        """
        L_orig, B_orig, C_orig = la.shape
        
        # 1. First convert to batch-first format
        la = la.permute(1, 0, 2)  # [B, L, D]
        
        # 2. Calculate spatial dimensions
        target_size = int(math.sqrt(L_orig))
        next_size = target_size + 1
        
        # 3. Choose appropriate spatial size
        if target_size * target_size == L_orig:
            spatial_size = target_size
        else:
            spatial_size = next_size
            
        # 4. Create padding
        total_spatial_elements = spatial_size * spatial_size
        padding_needed = total_spatial_elements - L_orig
        
        if padding_needed > 0:
            # Pad sequence dimension
            padding = torch.zeros(B_orig, padding_needed, C_orig, device=la.device, dtype=la.dtype)
            la_padded = torch.cat([la, padding], dim=1)
        else:
            la_padded = la
            
        # 5. Reshape to spatial format
        spatial_features = la_padded.reshape(B_orig, spatial_size, spatial_size, C_orig)
        
        # 6. Store original sequence length for later unpadding
        self._seq_len_cache = L_orig
        
        return spatial_features, spatial_size
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if active_adapter not in self.lora_variant:  # vanilla LoRA
                    ...
                    L, B, D = la.shape
                    # H = W = int(math.ceil(math.sqrt(L)))   # 24 (for 577) or 9 (for 77)
                    H = W = int(math.sqrt(L))

                    spatial_features, spatial_size = self.validate_and_reshape_lora(la)
                    ...
                    conv_flat = conv_flat.permute(1, 0, 2)  # [L, B, C]

                    # Continue with LoRA-B
                    # The same code is repeated below after the conv operation so dont un-comment this code...
                    # lb = lora_B(conv_flat)
                    # scaled_output = lb * scaling
                    

                    # print("Conv1 output shape:", conv_output.shape)  # Should be [1, 8, 13, 13] if input was [1, 8, 25, 25]
                    # print(HYE)

                    # OLD working uncomment code....

                    # L_orig, B_orig, C_orig = la.shape
                    # la = la.permute(1, 0, 2)  # [2, 577, 8]
                    # target_size = int(math.sqrt(L_orig))   # just root of the first value in la.shape

                    # target_spatial_size = target_size * target_size     #  576 (24 *24) (square root)
                    ...
                    # conv_flat = c1_op.permute(0, 2, 3, 1)
                    ...
                    # print("-----_____---___---__", conv_upsampled.shape, "*******8888****888***88****88**")
                    # print(lora_B.shape, "*******8888****888***88****88**")
                    # print(HEIYIO)

                    # lb = lora_B(conv_upsampled)
################################################################################################################################
                    
                    
                    # # la_res = dropout(x) @ lora_A.weight.T
                    # # print("LoraA output shape:", la.shape, "LORA res shape", la_res.shape)    # BOth are the same.....
                    # # print(HYE)
                    ...
                    # # if lora_res.numel() != la.numel():
                    # #     raise RuntimeError(
                    # #         f"Reshape operation invalid: input size {la.numel()} != output size {lora_res.numel()}. "
                    # #         f"Input shape: {la.shape}, Target shape: {[B_orig, target_size, target_size, C_orig]}"
                    # #     )
                    ...
                    # ###################Used with linear interpolation of LoRA##################
...
#############################################################################################################################
                    ...
                    print("_-____REsult and scaled_lora shapes---___---___", result.shape, scaled_lora.shape, "*******8888****888***88****88**")
                    # print(OIEHWOIH)

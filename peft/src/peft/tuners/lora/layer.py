...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        **kwargs,
        ...
        pattern = r'_([^\d]+?)_(0_\d+)'
        results = []
        count = 0
        sq_diff = 0
        sc_factor = lora_alpha/r
        for match in re.finditer(pattern, adapter_name):
            name = match.group(1)
            raw_value = match.group(2)
            value = float(raw_value.replace('_', '.'))
            results.append((name, value))


        for adapt_name, adapt_w in results:
        ...
        # if len(adapter_name)>25:
        #     sq_diff = math.sqrt((results[0][1]-results[1][1])**2 + (results[0][1]-results[2][1])**2 + (results[0][1]-results[3][1])**2 + (results[0][1]-results[4][1])**2)
            # import ipdb; ipdb.set_trace()


        # if len(adapter_name)>25:
        #     if 0.3 <= results[0][1] <= 0.9:
        #         sc_factor = 1.6
        #     elif 0.19 <= results[0][1] <= 0.29:
        #         sc_factor = 1.4
        #     else:
        #         sc_factor = 0.0
        ...
        if use_rslora:
            ...
            # else:
            #     self.scaling[adapter_name] = 1.2     # Overcomes the best result
            # self.scaling[adapter_name] = random.uniform(1.4, 1.7)
            # self.scaling[adapter_name] = sc_factor      # close to best but does not overcome best
            # if len(adapter_name)>=25:
                # scale_factor = (r*sq_diff) ** (int_sf)
                # scale_factor = 2 * int_sf

                ############# gives the best of 17.87 ##################
                # if count == 1:
                #     scale_factor = random.uniform(1.4, 1.7)
                # elif count == 2:
                #     scale_factor = random.uniform(1.2, 1.4)
                # elif count == 3:
                ...
                ################### 17.8715 ######################
                # if count == 1:
                #     scale_factor = random.uniform(1.5, 1.6)
                # elif count == 2:
                #     scale_factor = random.uniform(1.3, 1.499)
                # elif count == 3:
                #     scale_factor = random.uniform(1.1, 1.299)
                # elif count == 4:
            ...
            # else:
            #     scale_factor = lora_alpha / r
            # self.scaling[adapter_name] = scale_factor
            self.scaling[adapter_name] = nn.Parameter(torch.tensor(lora_alpha/r))

        # print("-_---___----_", self.scaling[adapter_name], "-__----___---__")
        # if len(adapter_name)>25:
        #     import ipdb; ipdb.set_trace()

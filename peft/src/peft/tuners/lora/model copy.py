...
##########################################################################################################
        ...
        #             # print('target_lora_B----',target_lora_B.shape)
        #             # target_conv1 = target.conv1.weight
        #         elif adapter_name in target.lora_embedding_A:
        #             target_lora_A = target.lora_embedding_A[adapter_name]
        #             target_lora_B = target.lora_embedding_B[adapter_name]
        #             if "conv" in adapter_name:
        ...
        #         target_lora_B.data = target_lora_B.data * 0.0
        #         if "conv" in adapter_name:
        #             target_conv1.data = target_conv1.data * 0.0
        #         # target_conv1 = target.conv1.weight * 0.0

        #         ############### 0: rain, 1: snow, 2: fog, 3: night ##################
        #         if combination_type == "cat":
        ...
        for key in key_list:
            ...
            if "conv" in adapter_name:
                ...
                if isinstance(target, LoraLayer):
                    ...
                    target_lora_A.data = target_lora_A.data * 0.0
                    target_lora_B.data = target_lora_B.data * 0.0
                    # target_conv1.data = target.conv1.weight.data* 0.0
                    ...
                    if combination_type == "cat":
                        ...
                        for adapter, weight in zip(adapters, weights):
                            if weight >= 0.25:
                                ...
                                if adapter in target.lora_A:
                                    current_adapter_lora_A = torch.zeros(target.lora_A[adapter].weight.shape).cuda()
                                    current_adapter_lora_B = torch.zeros(target.lora_B[adapter].weight.shape).cuda()
                                    if "conv" in adapter:
                                        current_adapter_conv1 = target.conv1[adapter].weight
                                elif adapter in target.lora_embedding_A:
                                    current_adapter_lora_A = torch.zeros(target.lora_embedding_A[adapter].shape)
                                    current_adapter_lora_B = torch.zeros(target.lora_embedding_B[adapter].shape)
                                    if "conv" in adapter:
                                        current_adapter_conv1 = target.conv1[adapter].weight
                                else:
                                    continue
                        ...
                        for adapter, weight in zip(adapters, weights):
                            if weight >= 0.25:
                                ...
                                if adapter in target.lora_A:
                                    ...
                                    current_adapter_lora_A = torch.zeros(target.lora_embedding_A[adapter].shape).cuda()
                                    current_adapter_lora_B = torch.zeros(target.lora_embedding_B[adapter].shape).cuda()
                                else:
                                    continue
                            loras_A.append(current_adapter_lora_A.data * weight * target.scaling[adapter])
                            loras_B.append(current_adapter_lora_B.data)

...
##########################################################################################################################
        ...
        if "conv" in adapter_name:
            if len(adapter_name)>25:
                ...
                target_name = re.search(r'_cat_(.*)', adapter_name)
                if "conv" in target_name.group(1):
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                target_adapter_name = re.search(r'_cat_(.*)', active_adapter)
                if "conv" in active_adapter and (not "conv" in target_adapter_name.group(1)):
                ...
                if active_adapter not in self.lora_variant:  # vanilla LoRA
                    ...
                    import ipdb; ipdb.set_trace()
                    if la.shape[0] == 5 and la.shape[2] == 577 and ("conv" in active_adapter) and (not "conv" in target_adapter_name.group(1)):

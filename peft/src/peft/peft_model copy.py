...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def get_base_model(self) -> torch.nn.Module:
        ...
        return self.base_model if self.active_peft_config_merging_tmp.is_prompt_learning else self.base_model.model
    ...
    def set_adapter(self, adapter_name: str, conv_adapter_name: str) -> None:
        """
...
################################################################
        ...
        # if adapter_name not in self.peft_config and conv_adapter_name not in self.peft_config:
        #     raise ValueError(f"Adapter {adapter_name} and {conv_adapter_name} not found.")
        
        # # Set both adapters as active
        # self.active_adapter = [adapter_name, conv_adapter_name]

        # # Handle non-prompt learning adapters
        # if not self.peft_config[adapter_name].is_prompt_learning:
        #     # Enable first adapter in base model
        #     self.base_model.set_adapter(adapter_name)
        #     # Enable second adapter in base model if different from first
        #     if conv_adapter_name != adapter_name:
        #         self.base_model.set_adapter(conv_adapter_name)
        # else:
        #     # Handle auxiliary modules for prompt learning
        #     _set_adapter(self, adapter_name)
        #     if conv_adapter_name != adapter_name:
        #         _set_adapter(self, conv_adapter_name)


######################################################################

        if adapter_name not in self.peft_config or conv_adapter_name not in self.peft_config:
            raise ValueError(f"Adapter {adapter_name} not found.")
        self.active_adapter = [adapter_name, conv_adapter_name]
        if not self.peft_config[adapter_name].is_prompt_learning:
            # _set_adapter does not need to be called, since it's called through the BaseTuner class.
            self.base_model.set_adapter(adapter_name)
        elif not self.peft_config[conv_adapter_name].is_prompt_learning:
            self.base_model.set_adapter(conv_adapter_name)
        else:
            # handle auxiliary modules
            _set_adapter(self, adapter_name)
    ...
    @property
    def active_peft_config_merging_tmp(self):
        import ipdb; ipdb.set_trace()
        return self.peft_config. self.active_adapter[0],  self.peft_config[self.active_adapter[1]]     # Error goes to line 984 in get_base_model()

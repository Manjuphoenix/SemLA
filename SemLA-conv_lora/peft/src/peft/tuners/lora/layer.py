...
##########################################################################################################################
        ...
        # if not (adapter_name == "default"):
        #     import ipdb; ipdb.set_trace(context=10)
        # print("----________----____----_____----_____-----________-----", adapter_name, "_-------____----____---______---_______-")

        if "conv" in adapter_name:
            # print(HYE)
            if len(adapter_name)>10:
                # print("-___----__---__lengthy adapter name-_____--____--__", adapter_name)
                ...
                # self.lora_B[adapter_name] = nn.Linear(r, self.out_features, bias=lora_bias)     # Scale up
                # import ipdb; ipdb.set_trace()
                # print("-___----__---adapter name-_____--____--__", adapter_name)
                self.conv1[adapter_name] = nn.Conv2d(r, 24, kernel_size=3, stride=2, padding=1, bias=False)
            self.conv1 = self.conv1.cuda()
        self.lora_bias[adapter_name] = lora_bias
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if active_adapter not in self.lora_variant:  # vanilla LoRA
                    ...
                    # import ipdb; ipdb.set_trace()
                    # if la.shape[0] == 577 and la.shape[1] == 5 and ("conv" in active_adapter):

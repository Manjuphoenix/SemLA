...
class CATSeg(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        print("-CLIP FEATURES 1 SHAPE- ----_______", clip_features["s1"].shape, "*88****888***888******")
        print("-CLIP FEATURES 2 SHAPE- ----_______", clip_features["s2"].shape, "*88****888***888******")
        # Process original scale features ONCE (these will be reused)
        original_res4 = None
        original_res5 = None


        if hasattr(self, 'layers') and len(self.layers) >= 2:
            # Process res4 and res5 from original scale (s1) - these have 24x24 spatial dimension
            original_res4 = rearrange(self.layers[0][1:, :, :], "(H W) B C -> B C H W", H=24, W=24)
            original_res5 = rearrange(self.layers[1][1:, :, :], "(H W) B C -> B C H W", H=24, W=24)
        ...
        # Define scale names and corresponding spatial dimensions
        scale_info = {
            "s1": {"H": 24, "layer_idx": 0},  # Assuming s1 corresponds to res3
            "s2": {"H": 36, "layer_idx": 1},     # s2 corresponds to res4 (layers[0])
            "s3": {"H": 12, "layer_idx": 2}      # s3 corresponds to res5 (layers[1])
        }

        for scale_name, scale_data in clip_features.items():

            # print("_--___----____--____--", scale_name, scale_data.shape)
            # Get target spatial dimensions for this scale

            print(f"Processing scale: {scale_name}")
            target_H, target_W = scale_info[scale_name]["H"], scale_info[scale_name]["H"]
            
            # Extract image features (removing CLS token)
            image_features = scale_data[:, 1:, :]
            
            # Create res3 equivalent for this scale
            # res3 = rearrange(image_features, "B (H W) C -> B C H W", H=scale_info[scale_name]["H"])
            # res3 = rearrange(image_features, "B (H W) C -> B C H W", H=scale_info[scale_name]["H"])
            res3 = rearrange(image_features, "B (H W) C -> B C H W", H=target_H, W=target_W)

            scale_features = {'res3': res3}

            if original_res4 is not None and original_res5 is not None:
                if scale_name == "s1":
                    # For original scale, use as-is
                    res4 = original_res4
                    res5 = original_res5
                else:
                    # For other scales, interpolate to match target spatial dimensions
                    target_size = (target_H, target_W)
                    res4 = F.interpolate(original_res4, size=target_size, mode='bilinear', align_corners=False)
                    res5 = F.interpolate(original_res5, size=target_size, mode='bilinear', align_corners=False)
                
                # Apply the upsampling layers
                res4 = self.upsample1(res4)
                res5 = self.upsample2(res5)
                
                # print(f"----_____---____---___---____ {res4.shape} Scaled clip features....")
                # print(f"----_____---____---___---____ {res5.shape} Scaled clip features....")
            else:
                res4 = res5 = None
            
            # Create features dictionary for this scale
            scale_features = {'res3': res3}
            if res4 is not None and res5 is not None:
                scale_features.update({'res4': res4, 'res5': res5})
                
            features_dict[scale_name] = scale_features


            # if scale_name in scale_layers and len(scale_layers[scale_name]) >= 2:
            #     # Option A: Use scale-specific intermediate layers
            #     res4_tokens = scale_layers[scale_name][0][1:, :, :].permute(1, 0, 2)  # Remove CLS, transpose
            #     res5_tokens = scale_layers[scale_name][1][1:, :, :].permute(1, 0, 2)  # Remove CLS, transpose
                
            #     res4 = rearrange(res4_tokens, "B (H W) C -> B C H W", H=target_H, W=target_W)
            #     res5 = rearrange(res5_tokens, "B (H W) C -> B C H W", H=target_H, W=target_W)
                
            # else:
            #     # Option B: Use original layers and interpolate to match target scale
            #     if hasattr(self, 'layers') and len(self.layers) >= 2:
            #         # Process original res4 and res5 (from s1 - 24x24)
            #         res4_original = rearrange(self.layers[0][1:, :, :], "(H W) B C -> B C H W", H=24, W=24)
            #         res5_original = rearrange(self.layers[1][1:, :, :], "(H W) B C -> B C H W", H=24, W=24)
                    
            #         # Interpolate to target scale dimensions
            #         target_size = (target_H, target_W)
            #         res4 = F.interpolate(res4_original, size=target_size, mode='bilinear', align_corners=False)
            #         res5 = F.interpolate(res5_original, size=target_size, mode='bilinear', align_corners=False)
            ...
            ###############################################################################################
            # res4 = rearrange(self.layers[0][1:, :, :], "(H W) B C -> B C H W", H=scale_info[scale_name]["H"])
            # res5 = rearrange(self.layers[1][1:, :, :], "(H W) B C -> B C H W", H=scale_info[scale_name]["H"])
            ...
            # Process res4 and res5 using scale-specific layers OR interpolation
            # if scale_name in scale_layers and len(scale_layers[scale_name]) >= 2:
            #     # Option A: Use scale-specific intermediate layers
            #     res4_tokens = scale_layers[scale_name][0][1:, :, :].permute(1, 0, 2)  # Remove CLS, transpose
            #     res5_tokens = scale_layers[scale_name][1][1:, :, :].permute(1, 0, 2)  # Remove CLS, transpose
            ...
            # # Initialize features dictionary for this scale
            # # scale_features = {'res3': res3}
            # # scale_features = {'res4': res4}
            # # scale_features = {'res5': res5}
            ...
            # Add res4 and res5 if available (from self.layers)
            # if scale_name in ["s2", "s3"] and hasattr(self, 'layers'):
            #     if scale_name == "s2" and len(self.layers) > 0:
            #         res4 = rearrange(self.layers[0][1:, :, :], "(H W) B C -> B C H W", H=24)
            #         res4 = self.upsample1(res4)
            #         scale_features['res4'] = res4
                    
            #     if scale_name == "s3" and len(self.layers) > 1:
            #         res5 = rearrange(self.layers[1][1:, :, :], "(H W) B C -> B C H W", H=24)
            #         res5 = self.upsample2(res5)
            #         scale_features['res5'] = res5
                    
            #         # Also add res4 for s3
            ...
            # scale_clip_features = {scale_name: scale_data}  # Create single-scale dict for sem_seg_head
            # outputs_dict[scale_name] = self.sem_seg_head(scale_clip_features, scale_features)

            print("----_____---____---___---____", features_dict[scale_name]["res3"].shape, "Scaled clip features....")
            print("----_____---____---___---____", features_dict[scale_name]["res4"].shape, "Scaled clip features....")
            print("----_____---____---___---____", features_dict[scale_name]["res5"].shape, "Scaled clip features....")
        print(HEY)
        # Final outputs dictionary containing all scales

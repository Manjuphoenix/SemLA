    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, LoraLayer):
                ...
                if adapter_name in target.lora_A and adapter_name in target.lora_conv1:
                    # import ipdb; ipdb.set_trace()
                    target_lora_A = target.lora_A[adapter_name].weight
                    # print('target_lora_A----',target)
                    # print('target_lora_A----',target_lora_A.shape)
                    target_lora_B = target.lora_B[adapter_name].weight
                    target_lora_conv1 = target.lora_conv1[adapter_name].weight
                ...
                if combination_type == "cat":
                    ...
                    for adapter, weight in zip(adapters, weights):
                        ...
                        if adapter in target.lora_A or adapter in target.lora_conv1:
                            # import ipdb; ipdb.set_trace()
                            current_adapter_lora_A = target.lora_A[adapter].weight
                            # print('current_adapter_lora_A----',current_adapter_lora_A.shape)
                            current_adapter_lora_B = target.lora_B[adapter].weight
                            current_adapter_conv1 = target.lora_conv1[adapter].weight

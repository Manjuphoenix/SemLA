...
class LoraLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.lora_conv1 = nn.ModuleDict({})
        # self.conv1 = self.conv1.cuda()

        # self.conv1.weight.requires_grad_(True)
        # if self.training:
        print("=== Conv1 parameters specifically ===")
        for name, param in self.lora_conv1.named_parameters():
...
##########################################################################################################################
        ...
        if "conv" in adapter_name:
            if len(adapter_name)>26:
                new_rank = adapter_name.count("conv1")*8
                ...
                self.lora_conv1[adapter_name] = nn.Conv2d(new_rank, 24, kernel_size=3, stride=2, padding=1, bias=False)
            else:
                # self.lora_A[adapter_name] = nn.Linear(self.in_features, r, bias=False)      # Scale down
                # self.lora_B[adapter_name] = nn.Linear(r, self.out_features, bias=lora_bias)     # Scale up
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                lora_A = self.lora_A[active_adapter]
                lora_B = self.lora_B[active_adapter]
                conv1 = self.lora_conv1[active_adapter]
                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
